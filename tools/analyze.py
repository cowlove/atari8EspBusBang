...
for line in d:
    ...
    if lastLine[clk] == 1 and line[clk] == 0:
        if lineNo > 1:
            ...
            if (ccount > 0 and ccount < 100): hist[4][ccount] = hist[4][ccount] + 1
    ...
    if line[extsel] == 0:
        #if lastLine[d0] != line[d0]: dTL = ccount
        #if dTE == -1 and lastLine[d0] != line[d0]: dTE = ccount
        if lastLine[d0] != line[d0]: hist[2][ccount] = hist[2][ccount] + 1
    #else:
...
for line in hist.T:
    if np.sum(line) > 0:
        print("%3d %6d %6d %6d %6d %6d" % (lineNo, line[0], line[1], line[2], line[3], line[4]))
    lineNo = lineNo + 1
